"""
Input-constrained minimum intervention QP-based safe control for unicycle dynamics using MultiBarriers.
Demonstrates MultiBarriers creation and input-constrained QP safe control synthesis.
"""
...
# Control bounds for unicycle dynamics [v_max, omega_max]
control_low = [-4.0, -1.0]  # [min linear velocity, min angular velocity]
control_high = [4.0, 1.0]   # [max linear velocity, max angular velocity]

# Make input-constrained safety filter
...
for i, traj in enumerate(trajs_list):
    ...
    # des_ctrl_vals = desired_control(traj, goal_single)
    ...
    min_barriers.append(map_composite.barrier.get_min_barrier_at(traj))
    min_constraint.append(map_composite.barrier.min_barrier(traj))
...
plt.savefig(os.path.join(script_dir, f'figs/06_Trajectories_Input_Constrained_QP_{current_time}.png'), dpi=200)
plt.show()
...
# Plot actions with control bounds
...
axs[3].legend(loc='lower center', ncol=3, frameon=False, fontsize=14)
axs[3].set_ylabel(r'$u_1$ (v)', fontsize=16)
...
axs[4].legend(loc='lower center', ncol=3, frameon=False, fontsize=14)
...
plt.savefig(os.path.join(script_dir, f'figs/06_States_Input_Constrained_QP_{current_time}.png'), dpi=200)
plt.show()
...
plt.savefig(os.path.join(script_dir, f'figs/06_Barriers_Input_Constrained_QP_{current_time}.png'), dpi=200)
plt.show()

# Print statistics about control bounds
print(f"Control statistics:")
print(f"  Linear velocity (u1): min={actions[0][:, 0].min():.3f}, max={actions[0][:, 0].max():.3f}")
print(f"  Angular velocity (u2): min={actions[0][:, 1].min():.3f}, max={actions[0][:, 1].max():.3f}")
print(f"  Bounds respected: u1 ∈ [{control_low[0]}, {control_high[0]}], u2 ∈ [{control_low[1]}, {control_high[1]}]")
