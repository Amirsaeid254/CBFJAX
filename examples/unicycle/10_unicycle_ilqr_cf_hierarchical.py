"""
Hierarchical iLQR + Closed-Form Safe Control for unicycle dynamics.
...
2. Low-level: MinIntervCFSafeControl filters the iLQR control
              to enforce CBF safety constraints (closed-form solution)

This approach combines:
- iLQR's ability to plan ahead and track references
- CF safety filter's efficient closed-form CBF enforcement
"""
...
# Barrier configuration for CF (uses composite barrier with HOCBF)
cfg = immutabledict({
    'softmax_rho': 20,
    'softmin_rho': 20,
...
print(f"{'='*60}")
print(f"Intervention statistics (CF modification):")
...
plt.savefig(os.path.join(script_dir, f'figs/10_iLQR_CF_Trajectory_{current_time}.png'), dpi=200)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'figs/10_iLQR_CF_States_{current_time}.png'), dpi=200)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'figs/10_iLQR_CF_Barrier_{current_time}.png'), dpi=200)
plt.show()
...
def create_animation():
    ...
    # Save animation
    animation_file = os.path.join(script_dir, f'figs/10_iLQR_CF_Animation_{current_time}.mp4')
