...
Demonstrates barrier function creation and safe control synthesis.
"""
...
print(jax.jit(map_.barrier.hocbf)(x0))
...
for i, traj in enumerate(trajs_list):
    ...
    actions.append(safety_filter.optimal_control(traj)[0])
...
plt.savefig(os.path.join(script_dir, f'figs/02_Trajectories_CF_Safe_Control_{current_time}.png'), dpi=200)
plt.show()
...
plt.savefig(os.path.join(script_dir, f'figs/02_States_CF_Safe_Control_{current_time}.png'), dpi=200)
plt.show()
...
# Plot barrier values
axs[0].plot(time_array, h_vals[0], color='black')
...
plt.savefig(os.path.join(script_dir, f'figs/02_Barriers_CF_Safe_Control_{current_time}.png'), dpi=200)
plt.show()
