...
min_barrier_at = jax.vmap(barrier.get_min_barrier_at)(x_hist)
...
plt.savefig(os.path.join(script_dir, f'figs/03_QP_Trajectory_{current_time}.png'), dpi=200)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'figs/03_QP_States_{current_time}.png'), dpi=200)
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(script_dir, f'figs/03_QP_Barrier_{current_time}.png'), dpi=200)
plt.show()
...
def create_animation():
    ...
    animation_file = os.path.join(script_dir, f'figs/03_QP_Animation_{current_time}.mp4')
