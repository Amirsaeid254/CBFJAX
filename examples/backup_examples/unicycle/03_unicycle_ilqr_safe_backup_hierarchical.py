"""
Hierarchical iLQR-Safe + Backup Safe Control for unicycle dynamics.
...
- Backup provides hard safety guarantees via backup policies
"""
...
# iLQR barrier configuration (simpler - just for cost penalty)
ilqr_barrier_cfg = immutabledict({
    'softmin_rho': 20.0,
    'boundary_alpha': (),
    'obstacle_alpha': (),
    'velocity_alpha': (),
...
map_ilqr = Map(dynamics=dynamics, cfg=ilqr_barrier_cfg, barriers_info=map_config)
...
Q = jnp.diag(jnp.array([0.01, 0.01, 0.001, 0.001]))
...
print(f"{'='*60}")
print(f"Intervention statistics (Backup modification of iLQR-safe):")
...
plt.tight_layout()
plt.savefig(f'figs/03_iLQR_Safe_Backup_Trajectory_{current_time}.png', dpi=300)
...
plt.tight_layout()
plt.savefig(f'figs/03_iLQR_Safe_Backup_States_{current_time}.png', dpi=300)
...
plt.tight_layout()
plt.savefig(f'figs/03_iLQR_Safe_Backup_Barriers_{current_time}.png', dpi=300)
...
def create_animation():
    ...
    # Save animation
    animation_file = f'figs/03_iLQR_Safe_Backup_Animation_{current_time}.mp4'
